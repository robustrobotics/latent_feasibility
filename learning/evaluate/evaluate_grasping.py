...
def get_pf_task_performance(logger, fname):
    ...
    for tx in eval_range:
        ...
        probs, labels = get_predictions_with_particles(resampled_parts, val_grasp_data, ensemble,
                                                       n_particle_samples=100)
...
def get_pf_validation_accuracy(logger, fname, amortize, debug):
    ...
    for tx in eval_range:
        ...
        if debug:
            ...
            labels = torch.as_tensor(val_grasp_data['grasp_data']['labels'])[:100]  # NOTE: can truncate here
        elif amortize:
