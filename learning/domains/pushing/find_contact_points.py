...
def find_contact_point_and_check_push(object_name, angle_degrees, push_velocity, object_mass, object_friction, com_shift, translation, gui=False, logging=False):
    ...
    sphere_id = p.loadURDF("sphere_small.urdf", [2 * math.cos(angle_radians), 2 * math.sin(angle_radians), 1], globalScaling=1)
    ...
    if contact_points:
        ...
        avg_contact_point = [sum(c[5][i] for c in contact_points) / len(contact_points) for i in range(3)]
...
def main():
    parser = argparse.ArgumentParser(description="Simulate object pushing and evaluate success.")
    parser.add_argument("--angle", type=float, default=90, help="Angle of push in degrees")
    parser.add_argument("--mass", type=float, default=0.00001, help="Mass of the objects")
    parser.add_argument("--friction", type=float, default=0.5, help="Friction of the objects")
    ...
    try:
        contact_point, push_successful, positions = find_contact_point_and_check_push(
            "Primitive::Box_3161924807452160_0.188m_0.274f_-0.012cx_0.035cy_-0.011cz.urdf", args.angle, args.velocity, args.mass, args.friction, com_shift, translation, args.gui, args.logging
        )
        ...
        print("The push was successful." if push_successful else "The push was not successful.")
