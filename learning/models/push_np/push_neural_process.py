...
class PushNP(nn.Module):
    ...
    def __init__(self, input_features: List[str], d_output: int, d_latents: int, activation: Optional[nn.Module] = None):
        super().__init__()
        ...
        self.N_PUSH_FEATURES = 5
        self.N_OBJ_FEATURES = 5

        # Layer sizes
        conv_sizes = [64, 256, 512]
        point_net_output_sz = 512
        linear_sizes = [conv_sizes[-1] + point_net_output_sz, 256, 128, d_latents * (d_latents + 1) // 2 + d_latents]
        decoder_sizes = [256, 512, d_output * (d_output + 1) // 2 + d_output]

        # Encoder layers
        self.point_net_encoder = PointNetRegressor(
            self.N_MESH_FEATURES, point_net_output_sz, n_geometric_features=self.N_GEOM_FEATURES
        )
        self.conv_layers = nn.ModuleList([
            nn.Conv1d(self.N_PUSH_FEATURES, conv_sizes[0], 1),
            nn.Conv1d(conv_sizes[0], conv_sizes[1], 1),
            nn.Conv1d(conv_sizes[1], conv_sizes[2], 1)
        ])
        self.linear_layers = nn.ModuleList([
            nn.Linear(linear_sizes[i], linear_sizes[i+1]) for i in range(len(linear_sizes)-1)
        ])

        # Decoder layers
        decoder_input_size = d_latents + self.N_PUSH_FEATURES + (self.N_OBJ_FEATURES if "com" in input_features else 0)
    ...
    def forward(self, x: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> Tuple[List[List[torch.distributions.MultivariateNormal]], torch.Tensor]:
        """
        ...
        tril_indices = torch.tril_indices(self.d_output, self.d_output, device=sigma.device)
        output = torch.zeros(input_vector.shape[0], input_vector.shape[1], self.d_output, self.d_output, device=sigma.device)
        output[:, :, tril_indices[0], tril_indices[1]] = sigma
        output[:, :, range(self.d_output), range(self.d_output)] = self.softplus(output[:, :, range(self.d_output), range(self.d_output)])

        result = [[torch.distributions.MultivariateNormal(mu[i][j], scale_tril=output[i][j]) for j in range(mu.shape[1])] for i in range(mu.shape[0])]
        return result, mu

    def get_latent_space(self, x: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> List[torch.distributions.MultivariateNormal]:
        """
        ...
        tril_indices = torch.tril_indices(self.d_latents, self.d_latents, device=variance_matrix.device)
        output = torch.zeros(input_vector.shape[0], self.d_latents, self.d_latents, device=variance_matrix.device)
        output[:, tril_indices[0], tril_indices[1]] = variance_matrix
        output[:, range(self.d_latents), range(self.d_latents)] = self.softplus(output[:, range(self.d_latents), range(self.d_latents)])

        return [torch.distributions.MultivariateNormal(mu[i], scale_tril=output[i]) for i in range(mu.shape[0])]