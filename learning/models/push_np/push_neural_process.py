...
class PushNP(nn.Module):
    ...
    def __init__(self, input_features, d_output, d_latents):
        ...
        self.decoder_layer_1 = nn.Linear(d_latents + n_push_features + (self.n_obj_features if 'com' in input_features else 0), decoder_layer_1_sz)
    ...
    def forward(self, x):
        ...
        zs = zs.expand(zs.shape[0], latent_size, push_data.shape[1])  # Now zs has shape [5, 10, 1]
        zs = zs.swapaxes(1, 2)
        ...
        output = torch.zeros(
            (input_vector.shape[0], input_vector.shape[1], self.d_output, self.d_output),
            device=sigma.device,
        )
        ...
        for i in range(input_vector.shape[0]):
            for k in range(input_vector.shape[1]):
                ...
                output[i, k, diag_indices, diag_indices] = self.softplus(output[i, k, diag_indices, diag_indices])

        # print(f"mu sz: {mu.shape}")
        # print(f"output sz: {output.shape}")
        result = [[torch.distributions.MultivariateNormal(mu, scale_tril=output[i][j]) for j in range(mu.shape[1])] for i in range(mu.shape[0])]
        return result 
    ...
    def get_latent_space(self, x):
        ...
        for i in range(results.shape[0]):
            ...
            output[i, diag_indices, diag_indices] = self.softplus(output[i, diag_indices, diag_indices])
