...
class CustomGraspNeuralProcess(nn.Module):
    ...
    def __init__(self, d_latents):
        ...
        self.decoder = CustomGNPDecoder(n_in=3+n_out_geom+d_latents+d_mesh, d_latents=d_latents)
    ...
    def forward(self, contexts, target_xs, meshes):
        ...
        context_geoms, context_midpoints, context_labels = contexts
        ...
        mu, sigma = self.encoder(geoms_enc, context_midpoints, context_labels, mesh_enc)
        ...
        geoms_enc = self.grasp_geom_encoder(geoms).view(n_batch, n_grasp, -1)
        y_pred = self.decoder(geoms_enc, target_mids, z, mesh_enc)
        return y_pred, q_z
    ...
    def conditional_forward(self, target_xs, meshes, zs):
        ...
        y_pred = self.decoder(geoms_enc, target_mids, zs, mesh_enc)
        return y_pred
...
class CustomGNPDecoder(nn.Module):
    ...
    def forward(self, target_geoms, target_midpoints, zs, meshes):
        ...
        xs_with_latents = torch.cat([target_midpoints, target_geoms, zs_broadcast, meshes_broadcast], dim=2)
...
class CustomGNPEncoder(nn.Module):
    ...
    def forward(self, geoms_enc, context_midpoints, context_labels, meshes):
        """
